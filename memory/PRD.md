...
- **2026-05-12 — Twilio volledig vervangen door SMSTools (EU SMS provider)**
  ...
  - **Removed**: `clients/twilio.py` volledig verwijderd uit codebase.
  - **Settings model** (`routers/admin.py::PlatformSettingsUpdate`): `twilio_*` velden volledig vervangen door `smstools_client_id`, `smstools_client_secret`, `smstools_sender_id`, `smstools_whatsapp_sender`, `smstools_test_mode`, `smstools_webhook_url`. Client secret Fernet-encrypted at rest. GET response stript legacy `twilio_*` velden zodat de UI ze niet meer rendert.
  - **Integrations endpoint** (`GET /admin/integrations`): nieuw veld `smstools` met `configured` + `balance` (best-effort GET /balance). `twilio` veld behouden als deprecated alias (verwijst naar zelfde data) zodat ge-cachete UI builds niet crashen tijdens de rolling deploy.
  - **Notification dispatcher** (`services/notifications_sms.py`): imports omgezet naar `clients.smstools`, exception type `TwilioError` → `SMSToolsError`. DB field `twilio_sid` → `provider_message_id` + `provider: "smstools"`. Refund-flow ongewijzigd.
  - **Webhook handler**: `handle_twilio_status` → `handle_smstools_status` — leest beide `provider_message_id` ÉN legacy `twilio_sid` voor backwards compat met bestaande records. Endpoint `POST /api/notifications/twilio/status` → `POST /api/notifications/smstools/status`, accepteert nu zowel JSON (SMSTools default) als form-encoded body.
  - **Admin UI** (`Admin.jsx`): Twilio settings sectie volledig vervangen door **"SMSTools (EU SMS + WhatsApp gateway)"** met velden voor Client ID, Client Secret, Sender ID, WhatsApp sender, Delivery webhook URL, Test mode. Top-level Twilio status-pill in Overview vervangen door SMSTools (met optionele balance display). **Nieuwe `SMSToolsTestSend` widget** onder de sectie: input voor recipient (E.164) + channel-select (SMS/WhatsApp) + "Send test" knop. Praat direct met de admin endpoint `POST /admin/smstools/test` zonder credit-verbruik.
  - **Test endpoint**: `POST /api/admin/smstools/test` — admin-only, doet één live `send_sms()`/`send_whatsapp()` met platform balance (geen workspace credits charged), audit-logged met geredacteerd telefoonnummer.
  - **Migratie-pad voor user**: in Cloudflare Dashboard → `*.deployunit.app` Origin Cert al klaar → SMSTools account aanmaken op smstools.com → API keys kopiëren → in Admin → Platform Domain → SMSTools-sectie invullen → save → klik "Send test" met je eigen nummer → SMS arriveert in 5-10s.
  - **Tests**: curl tests groen — integrations endpoint toont SMSTools status, PUT settings persist Client ID, Client Secret Fernet-encrypted, test endpoint praat correct met SMSTools API (returnt code 104 met dummy creds → bewijs dat de auth header correct gestuurd wordt). Lint groen op alle 5 gewijzigde files.
...
- **2026-05-13 — Iter18 — No-JS site loading + Safe custom subdomain (P1)**
  - **Feature 1 — Prerender public pages**: Added `/app/frontend/scripts/prerender.py` (Python + Playwright Chromium) as a `postbuild` step. Visits every public marketing route in a headless browser and writes the fully-rendered HTML to `build/<route>/index.html`. Static hosts then serve real content for `/`, `/pricing`, `/about`, `/contact`, `/support`, `/status`, `/login`, `/register`, `/forgot-password` even without JavaScript. SPA `index.js` now uses `hydrateRoot` when prerendered content is present so React 19 picks up where the snapshot left off — zero impact on JS visitors, instant readable page for everyone else. Normalizes localhost origin → `https://deployunit.com` and dedupes the PostHog snippet. Postbuild gracefully no-ops if Chromium / Playwright is missing in the build env.
  - **Feature 2 — Custom subdomain with DNS verification**: New service `/app/backend/services/custom_subdomain.py` with the full state machine (`pending` → `active` | `failed` | `cancelled` | `detached`). Validates 3–63 char lowercase-alnum-hyphen names, blocks 60+ reserved words, checks pool + active-claim conflicts, then creates the Cloudflare DNS record, adds the FQDN to Coolify Traefik (alongside the existing random one — old URL stays online), and queues a 30s scheduler tick that probes HTTPS until 3 consecutive 2xx/3xx responses confirm the route is live. Only then does the primary URL switch and the old random subdomain return to the pool. 10-min hard timeout + 6-failure threshold trigger automatic rollback (DNS record + Coolify domain both cleaned up).
  - **API endpoints** (member-scoped): `GET /api/apps/{id}/custom-subdomain` (state), `GET /api/apps/{id}/custom-subdomain/check?name=...` (real-time availability), `POST /api/apps/{id}/custom-subdomain` (request), `DELETE /api/apps/{id}/custom-subdomain` (cancel pending OR detach active and restore old URL).
  - **Frontend**: New `CustomSubdomainCard` component in AppDetail → Domains tab. Debounced availability check while typing, suffix preview (`.deployunit.com`), pending state with progress bar (probe_success / probe_success_needed), active state with detach button, failed-state error banner. Auto-polls every 5s while pending.
...
- **2026-05-13 — Iter22c — "Coming soon" labels removed for Heatmaps (P0 UX)**
  - **User report (Dutch)**: "heatmaps staat nog steeds op coming soon? klopt dat?" — even though iter22 shipped the in-house heatmaps add-on, stale "soon" placeholders survived in several places.
  - **Fixes**:
    * `components/AppWebAnalyticsTab.jsx` — removed the `soon: true` + `gated: "heatmaps"` from the Analytics → Heatmaps SUB_TABS entry; fully rewrote `HeatmapsPane` to call `/api/apps/{id}/addons`. If inactive → renders an "Enable for 100 cr / mo" CTA (data-testid `heatmaps-enable-btn`) wired to the addon enable endpoint; if active → renders "Heatmaps are live" with stat tiles (Total events / Clicks / Rage clicks), a top-8 URL summary table, and an "Open full viewer" button (data-testid `heatmaps-open-full-viewer`) that navigates to `/app/apps/:id/heatmaps`. Subtitle no longer says "Microsoft Clarity heatmaps" — now "in-house click heatmaps".
    * `pages/Landing.jsx` — analytics bento `<TabbedCard>` Heatmap inner-tab no longer carries the `soon: true` flag; copy block changed from "Heatmaps and session replays land next" to "In-house click & rage-click heatmaps — no third-party scripts."; `HeatmapPreview` top-right badge swapped from "soon" → "live"; Roadmap teaser tile "Native heatmaps & session replays" → "Session replays (rrweb)" (only session replays remain on the roadmap).
...
- **2026-05-13 — Iter23b — Auto-injector observability fix (P0)**
  - **User report**: "Ik kan nog niet verifiëren dat de auto injection heeft gewerkt … ik zie nog geen resultaten in heatmaps of analytics." Build logs only showed DeployUnit's own `[BUILD] analytics auto-inject ON` line — there was no way to know whether (a) Coolify actually accepted our PATCH, (b) the build container reached our preflight URL, or (c) the preflight ran but matched no framework.
  - **Fix — added observability at three layers**:
    1. **Deploy log read-back**: after PATCHing `build_command` in `_redeploy_background`, we now call `coolify.get_application(uuid)` and verify the stored command contains `"preflight.js"`. The deploy log prints one of three explicit lines: `[BUILD] auto-inject: Coolify stored the wrapped build command (verified by read-back)` / `[WARN] auto-inject: Coolify rejected the wrapped command (returned: <head>)` / `[WARN] auto-inject: Coolify accepted the PATCH but the build_command read-back is empty`.
    2. **GET /api/apps/{id}/auto-inject** now returns `coolify_build_command`, `coolify_has_preflight`, and `coolify_error` so the Setup tab can show live build-engine state.
    3. **Setup tab UI** — new "build engine state" strip (data-testid `auto-injector-coolify-state`) with 4 explicit states: ✓ stored, ⚠ stored but no preflight (with the actual command), ✗ Coolify read failed, · no build engine link yet.

      ...
      <div className="flex items-center justify-center p-8 order-2 lg:order-1">
        <div className="w-full max-w-sm">
          ...
          <form onSubmit={submit} className="mt-4 space-y-4" data-testid="register-form">
            ...
            <div>
              <label className="text-[10px] uppercase tracking-[0.3em] text-zinc-500 font-mono">Password (min 6)</label>
              <input
          ...
          <div className="mt-5 text-xs font-mono text-zinc-500 flex justify-between">
            <Link to="/login" className="hover:text-white" data-testid="link-login">Already have an account? Sign in →</Link>
            <Link to="/" className="hover:text-white">← Home</Link>
          </div>
        </div>
      </div>

      <div className="hidden lg:block relative bg-grid noise overflow-hidden border-l border-white/[0.06] order-1 lg:order-2">
        <div className="absolute inset-0 bg-gradient-to-bl from-brand/10 via-transparent to-transparent" />
        <div className="relative h-full flex flex-col justify-between p-12">
          <Link to="/"><Logo /></Link>
          <div>
            <div className="text-xs font-mono uppercase tracking-[0.3em] text-brand mb-3">// what's inside</div>
            <h2 className="font-display text-4xl font-semibold tracking-tighter leading-tight max-w-md">
              Production hosting,<br />zero config.
            </h2>
            <ul className="mt-6 space-y-2 text-sm text-zinc-300 max-w-md">
              {[
                ...
                <li key={t} className="flex items-center gap-2"><Check className="h-4 w-4 text-brand" />{t}</li>
              ))}
            </ul>
          </div>

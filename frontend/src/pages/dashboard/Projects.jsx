...
export default function Projects() {
  ...
  return (
    <div className="px-6 py-6" data-testid="clients-page">
      <div className="flex items-end justify-between mb-6">
        <div>
          <div className="text-xs font-mono uppercase tracking-[0.3em] text-brand mb-2">// clients</div>
          <h1 className="font-display text-4xl font-semibold tracking-tighter">Group your apps</h1>
          <p className="mt-1 text-sm text-zinc-400">Use clients to group apps per client or initiative. Great for agencies.</p>
        </div>
        <button
      ...
      {loading ? (
        <div className="text-zinc-500 font-mono text-sm">Loading clients…</div>
      ) : projects.length === 0 ? (
        <div className="border border-dashed border-white/10 p-16 text-center" data-testid="empty-clients">
          <FolderKanban className="h-10 w-10 text-zinc-600 mx-auto mb-4" />
          <h3 className="font-display text-2xl">No clients yet</h3>
          <p className="mt-2 text-zinc-400">Create a client to group apps together.</p>
        </div>
      ) : (
        <div className="border-t border-l border-white/[0.06] grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3">
          {projects.map((p) => (
            <Link key={p.id} to={`/app/projects/${p.id}`} className="p-6 border-r border-b border-white/[0.06] hover:bg-white/[0.02] transition group" data-testid={`client-${p.slug}`}>
      ...
      {open && (
        <div className="fixed inset-0 bg-black/70 z-50 flex items-center justify-center p-4">
          <div className="bg-elevated border border-white/10 max-w-md w-full p-6 relative animate-rise">
            ...
            <h3 className="font-display text-2xl">New client</h3>
            <form onSubmit={create} className="mt-5 space-y-4">
              ...
              <div>
                ...
                <textarea
                  ...
                  placeholder="What this client covers..."
                />
              </div>
              {error && <div className="text-signal-failed text-sm">{error}</div>}
              <div className="flex gap-2">
                <button type="submit" className="flex-1 py-2 bg-brand text-brand-fg font-medium hover:bg-brand/90" data-testid="client-create-submit">Create</button>
